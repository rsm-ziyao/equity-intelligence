...
from ..dependencies import get_db_session, get_fundamentals_service, get_financial_trend_service, get_financial_analysis_service, get_valuation_service
from ..schemas.fundamentals import FinancialHistoryResponse, FundamentalsResponse
from ..schemas.analysis import FinancialAnalysisResponse
...
@router.get("/{symbol}/fundamentals/analysis", response_model=FinancialAnalysisResponse)
def get_financial_analysis(
    symbol: Annotated[str, Path(min_length=1, max_length=10, pattern=r"^[A-Za-z][A-Za-z0-9.-]{0,9}$")],
    period_type: str = Query("annual"),
    limit: int = Query(8, ge=1, le=20),
    session: Session = Depends(get_db_session),
    service: FinancialAnalysisService = Depends(get_financial_analysis_service),
):
    try:
        result = service.get_analysis(session, symbol, period_type.lower(), limit)
    except ValueError as exc:
        raise HTTPException(status_code=422, detail=str(exc)) from exc
...
def get_valuation(
    ...
    try:
        result = service.get_valuation(session, symbol)
    except ValueError as exc:
        raise HTTPException(status_code=422, detail=str(exc)) from exc
